...
```text
XXX.XXX.XXX.XXX cp.larstechnologies.home cp
XXX.XXX.XXX.XXX n1.larstechnologies.home node-1 10.200.0.0/24
XXX.XXX.XXX.XXX n2.larstechnologies.home node-2 10.200.1.0/24
```
...
In this section you will generate and distribute an SSH keypair to the `cp`, `node-1`, and `node-2`, machines, which will be used to run commands on those machines throughout this tutorial. Run the following commands from the `jumpbox` machine.
...
In this section you will assign hostnames to the `cp`, `node-1`, and `node-2` machines. The hostname will be used when executing commands from the `jumpbox` to each machine. The hostname also plays a major role within the cluster. Instead of Kubernetes clients using an IP address to issue commands to the Kubernetes API server, those clients will use the `cp` hostname instead. Hostnames are also used by each worker machine, `node-1` and `node-2` when registering with a given Kubernetes cluster.
...
In this section you will generate a `hosts` file which will be appended to `/etc/hosts` file on the `jumpbox` and to the `/etc/hosts` files on all three cluster members used for this tutorial. This will allow each machine to be reachable using a hostname such as `cp`, `node-1`, or `node-2`.
...
XXX.XXX.XXX.XXX cp.larstechnologies.home cp
XXX.XXX.XXX.XXX node-1.larstechnologies.home node-1
XXX.XXX.XXX.XXX node-2.larstechnologies.home node-2
```
...
# Kubernetes The Hard Way
XXX.XXX.XXX.XXX cp.larstechnologies.home cp
XXX.XXX.XXX.XXX n1.larstechnologies.home node-1
XXX.XXX.XXX.XXX n2.larstechnologies.home node-2
```

At this point you should be able to SSH to each machine listed in the `machines.txt` file using a hostname.

```bash
for host in cp node-1 node-2
...
At this point, hostnames can be used when connecting to machines from your `jumpbox` machine, or any of the three machines in the Kubernetes cluster. Instead of using IP addresses you can now connect to machines using a hostname such as `cp`, `node-1`, or `node-2`.
